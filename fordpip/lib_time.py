...
def date_str_with_time(str_date, str_time='00:00:00', s=None):
    '''
    Return date with time in a default format.
    '''
    try: # 29/05/2016
        d = int(str_date[:2])
        d = str_date[:2]
    ...
    # get time if in date string
    if len(str_date) == 16:
        str_time = str_date[11:]+':00'
    elif len(str_date) == 19:
        str_time = str_date[11:]

    # set default split character
    x = s if isinstance(s,str) else x

    # set new date string
    return (str(y)+x+str(m)+x+str(d)+' '+str_time)
...
def datetime_from_timestamp(timestamp, tz=None, utc=False):
    ...
    Setting the "utc" flag to True will set parsing datasets
    using +0 as default time zone instead of local machine time.
    '''
    if tz == 0 or utc: # universal time coordinates
        return datetime.utcfromtimestamp(timestamp)
    elif isinstance(tz, int): # add seconds
        timestamp = int(timestamp + tz)
...
def get_time_elapsed(start_time, time_as_int=False):
    '''
    Return time elapsed since start time.
    '''
    end_time = int(time()) if time_as_int else time()
...
def set_time_zone(seconds=None):
    '''
    Return seconds as int for time zone or local time.
    '''
